<div class="md:flex md:h-96">
    ...
    <div class="flex flex-col justify-between p-4 md:w-1/2">
        <div>
            <a class="mt-4 text-blue-600 text-md" wire:navigate href="{{route('categories.show', ['category' => $category->id])}}">{{ $category->category }}</a>
            <h2 class="mt-2 text-3xl font-semibold text-gray-800">{{ $product->name }} <span class="text-sm text-blue-400"></h2>
            <p class="text-blue-600"> 10% OFF</p>
            
            <p class="mt-2 text-4xl text-gray-900">${{ $product->price }}</p>
            <p class="mt-4 text-gray-500 text-md">{{ $product->description }}</p>
        ...
        @if(auth()->check() && auth()->user()->admin === 1)
            <div class="my-2 mt-auto ml-auto">
                <div class="flex flex-row m-5">
                    <a wire:navigate href="{{route('products.edit', ['product' => $product->id])}}" class="px-2 mr-2">
