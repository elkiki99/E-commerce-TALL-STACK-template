    ...
    <div class="relative">
        <div class="lg:flex">
            ...
            <div class="flex flex-col justify-between p-4 lg:w-1/2">
                <div>
                    <a class="mt-4 text-blue-600 text-md" href="{{ route('categories.show', ['category' => $category->id]) }}">{{ $category->category }}</a>
                    <h2 class="mt-2 text-3xl font-semibold text-gray-800">{{ $product->name }} <span class="text-sm text-blue-400"></span></h2>
                    <p class="text-blue-600">10% OFF</p>
                    <p class="mt-2 text-4xl text-gray-900">${{ $product->price }}</p>
                                        
                    <div class="flex flex-wrap mt-4">
                        @foreach ($tags as $tag)
                            <a href="{{ route('tags.show', ['tag' => $tag->id]) }}" class="inline-block px-3 py-1 mb-2 mr-2 text-sm font-semibold text-gray-700 bg-gray-200 rounded-full">
                                {{ $tag->tag }}
                            </a>
                        @endforeach
                    </div>
                    
                    <p class="mt-4 text-gray-500 break-words text-md">{!! $product->description !!}</p>
                </div>
                    ...
                    <div class="my-2 mt-auto ml-auto">
                        <div class="flex flex-row m-5">
                            <a href="{{ route('products.edit', ['product' => $product->id]) }}" class="px-2 mr-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mt-2 text-blue-600">
