<div class="flex flex-col">
    <table class="hidden w-full lg:table">
        ...
        <tbody>
            @foreach ($products as $product)
                <tr class="border-t-2 border-gray-200 dark:border-gray-600" x-data="{ quantity: {{ $product['quantity'] }}, price: {{ $product['product']->price }}, total: {{ $product['product']->price * $product['quantity'] }} }">
                    <td class="py-2">
    ...
    <div class="flex flex-col w-full text-gray-900 dark:text-gray-100 lg:hidden">
        <div class="justify-center w-full">
            @foreach ($products as $product)
                <div class="flex w-full border-t-2 border-gray-200 dark:border-gray-600"
                    x-data="{ quantity: {{ $product['quantity'] }}, price: {{ $product['product']->price }}, total: {{ $product['product']->price * $product['quantity'] }} }" wire:key="product-{{ $product['product']->id }}">
        
                    <img src="{{ asset('storage/img/products/' . $product['product']->image ) }}" alt="{{ $product['product']->name }}" class="flex-shrink-0 m-2 h-36 w-36"></img>
                    <div class="w-full mt-2 ml-4">
                        <div class="flex items-center justify-between w-full">
                            <p class="text-xl font-bold dark:text-gray-300">{{ $product['product']->name }}</p>
        
                            <button wire:click="remove({{ $product['product']->id }})" wire:key="remove-{{ $product['product']->id }}"
                        ...
                        <div class="flex flex-grow w-full mt-2">
                            <div>
                                <button x-on:click="quantity = quantity > 1 ? quantity - 1 : quantity; total = (quantity * price).toFixed(2)"
                                    wire:click="update({{ $product['product']->id }}, quantity > 1 ? quantity - 1 : quantity)">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                        stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" />
                            ...
                            <div>
                                <button x-on:click="quantity++ ; total = (quantity * price).toFixed(2)"
                                    wire:click="update({{ $product['product']->id }}, quantity + 1)">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                        stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        ...
                        <p x-text="total" class="text-lg font-bold dark:text-gray-300">
                            <span class="font-bold dark:text-gray-100"></span>
                        </p>
                    </div>
                </div>
            @endforeach
